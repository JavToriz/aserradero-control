...
type FormData = {
  ...
  clave_sat?: string; // 👇 NUEVO: Tipo de dato
  precio_venta: number;
...
export const ProductForm = ({ productType, initialData, onSave, onCancel }: ProductFormProps) => {
  ...
  const calcularPrecio = async (genero: string, clasificacion: string, grosor: number, ancho: number, largo: number) => {
    ...
    try {
      ...
      const res = await fetch('/api/precios/calcular', {
        ...
        body: JSON.stringify({ genero, clasificacion, grosor, ancho, largo })
      });
  ...
  useEffect(() => {
    if (initialData) {
      ...
      reset({ 
         ...
         clave_sat: productData.clave_sat || '', // 👇 NUEVO: Cargar si existe
  ...
  const processForm = async (data: FormData) => {
    const productData: any = {
      ...
      clave_sat: data.clave_sat, // 👇 NUEVO: Enviar en el payload
      unidad_medida: 'Pieza', 
  ...
  return (
    <form onSubmit={handleSubmit(processForm)} className="space-y-8">      
      ...
      <div className="bg-gray-50 p-6 rounded-lg border border-gray-200 shadow-sm">
        ...
        {/* Modificamos la cuadrícula para que la Descripción siga ocupando todo el ancho, y abajo queden en fila SKU, Código Barras y Clave SAT */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="col-span-1">
            ...
            <p className="text-xs text-gray-500 mt-1">Incluye sufijo aleatorio para evitar duplicados.</p>
          </div>
          ...
          <div className="col-span-1">
            ...
            <input 
              ...
              className="block w-full px-4 py-2 bg-white border border-gray-300 rounded-lg shadow-sm text-gray-800 font-mono text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all" 
            />
            <p className="text-xs text-gray-500 mt-1">Opcional. Usa tu escáner aquí.</p>
          </div>
          ...
          <div className="col-span-1">
            ...
            <input 
              ...
              className="block w-full px-4 py-2 bg-white border border-gray-300 rounded-lg shadow-sm text-gray-800 font-mono text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all" 
            />
            <p className="text-xs text-gray-500 mt-1">Opcional. Catálogo de productos/servicios.</p>
          </div>

          {/* PRECIOS CALCULADOS */}
          <div className="col-span-full grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-gray-200 mt-2">
            <div>
