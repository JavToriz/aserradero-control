// src/app/(dashboard)/productos/[categoria]/page.tsx
'use client';
...
export default function ProductsPage() {
  ...
  // Validar parámetro de URL y derivar categoría
  ...
  const activeTab: 'MADERA_ASERRADA' | 'TRIPLAY_AGLOMERADO' = inMadera ? 'MADERA_ASERRADA' : 'TRIPLAY_AGLOMERADO';
  ...
  useEffect(() => {
    // Solo hacemos fetch si la categoría es válida
    if (inMadera || inTriplay) {
      fetchProducts();
      setCurrentPage(1); 
      setSearchTerm(''); 
      setModoSimulacion(false); // Apaga el simulador al cargar/cambiar listado
    }
  }, [fetchProducts, inMadera, inTriplay]);
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">
            {inMadera ? 'Catálogo de Madera Aserrada' : 'Catálogo de Productos / Triplay'}
          </h1>
          <p className="text-gray-500">Define descripciones, códigos y precios de venta.</p>
        </div>
        <Link href={`/productos/${categoriaParam}/nuevo`} passHref className="w-full md:w-auto">
